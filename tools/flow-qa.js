...
(async()=>{
  ...
  // 2. closed door physically blocks crossing
  await page.keyboard.press('ArrowUp');
  await sleep(700);
  const afterWrong = await page.evaluate(()=>({lives:window.game.lives,score:window.game.score,doorState:window.game.door.state,blocking:window.game.door.isBlocking()}));
  if(afterWrong.lives !== 2 || afterWrong.score !== 0 || afterWrong.doorState !== 'locked' || afterWrong.blocking !== false){console.error('FAIL: 2. closed door blocks crossing, got:', afterWrong); process.exit(1);}
  ...
  // 4. wrong A: lives === 2 score === 0 door.state === locked
  ...
  await sleep(2200);
  const afterCorrect = await page.evaluate(()=>({score:window.game.score,doorState:window.game.door.state,blocking:window.game.door.isBlocking()}));
  ...
  if(afterCorrect.blocking !== false){console.error('FAIL: 6. correct B: isBlocking=false, got:', afterCorrect.blocking); process.exit(1);}
  ...
  if(startBtn){
    ...
    if(s.lives !== 3 || s.score !== 0){console.error('FAIL: 12. second run: lives=3 score=0, got:', s); process.exit(1);}
  }
  console.log('12. second run: lives=3 score=0: PASS');
